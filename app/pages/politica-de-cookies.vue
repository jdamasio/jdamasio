<template>
  <div v-if="pageData" id="page-politica-de-cookies">
...
export default {
  async fetch({ store }) {
    try {
      if (store.getters['cookies/getCookiesData'] === null)
        await store.dispatch('cookies/getPageData')
    } catch (e) {
  ...
  computed: {
    ...mapGetters({
      pageData: 'cookies/getCookiesData'
    })
  }
}
</script>
